...
/*copies data from file into array of structs*/
struct _data *LOAD(FILE *stream, int wordcount) {
   ...
   for(index = 0; index < wordcount; index++){  
         fscanf (stream, "%s", tempstr);
         if ((tempstr[0] >= 33 && tempstr[0] <= 47) ||(tempstr[0] >= 58 && tempstr[0] <= 63)){ //if 1stchar is sym
            for (in3 = 0; in3 <  50; in3++){
               tempstr[in3] = tempstr[in3+1];
            }
         }
         for (in = 0; in < 50; in++){ //This removes punctuation & other symbols
            if ((tempstr[in] >= 33 && tempstr[in] <= 47) ||(tempstr[in] >= 58 && tempstr[in] <= 63)){
               tempstr[in] = '\0';
            }
         }
         for (in2 = 0; in2 < 50; in2++){
            if (tempstr[in2] >= 'A' && tempstr[in2] <= 'Z'){ //This sets all uppercase -> lowercase
